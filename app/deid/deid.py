    ...
    for i in range(0, 10):
        try:
            producer = SerializingProducer(
                kafka,
                schema_registry=avro_serializer,
...
def retrieve_schema(schema_name):
    ...
    for field in schema_info['entity']['attributes']['fields']:
        ...
        #Check if the field is PII or not
...
def run_deid_pipeline(schema_name,topic_input,topic_output,key_name ):
        ...
        for key in message.value.keys():
            ...
            if len(pii_entities):
                ...
                if len(pii_entities) > 1: # Free text field
                    ...
                    analyzer_results=[
                        OperatorResult(entity_type=pii_entities[0], start=0, end=len(str(message.value[key])), score=1 )
                    ]
